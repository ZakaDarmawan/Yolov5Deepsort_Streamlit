...
def detect(opt, stframe, mobil, bus, truk, motor, line, fps_rate, class_id):
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)):
                        ...
                        # count_obj(bboxes,w,h,id, names[c], data_mobil, data_bus, data_truk, data_motor)
            ...
            if show_vid:
                ...
                fontScale = 2
                # cv2.putText(im0, 'mobil: ' + str(len(mobil)), org, font, fontScale, mobil, thickness, cv2.LINE_AA)
                # cv2.putText(im0, 'bus: ' + str(len(data_bus)), (org[0], org[1] + distance_height), font, fontScale, color_bus, thickness, cv2.LINE_AA)
                # cv2.putText(im0, 'truk: ' + str(len(data_truk)), (org[0], org[1] + distance_height*2), font, fontScale, color_truk, thickness, cv2.LINE_AA)
            ...
            if save_vid:
                ...
                mobil.markdown(f"<h3> {str(len(data_mobil))} </h3>", unsafe_allow_html=True)
                bus.write(f"<h3> {str(len(data_bus))} </h3>", unsafe_allow_html=True)
                truk.write(f"<h3> {str(len(data_truk))} </h3>", unsafe_allow_html=True)
...
default_model_path = os.path.join(script_dir, 'https://github.com/ZakaDarmawan/Yolov5Deepsort_Streamlit/blob/master/yolov5/runs/train/exp/weights/best.pt')

def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--yolo_model', nargs='+', type=str, default='https://github.com/ZakaDarmawan/Yolov5Deepsort_Streamlit/blob/master/yolov5/runs/train/exp/weights/best.pt', help='model.pt path(s)')
